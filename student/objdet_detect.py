...
def detect_objects(input_bev_maps, model, configs):
        ...
        if 'darknet' in configs.arch:
            ...
            for sample_i in range(len(output_post)):
                ...
                for obj in detection:
                    ...
                    x1, y1, x2, y2, conf, cls_conf, cls_pred = obj
                    x = (x1 + x2) / 2.0
                    y = (y1 + y2) / 2.0
                    w = x2 - x1
                    l = y2 - y1
                    
                    # append the object to the detections list
                    detections.append([cls_pred.item(), x, y, 0.0, 1.50, w, l, 0.0])

            
            ####### ID_S3_EX1-5 START #######     
            #######
            print("student task ID_S3_EX1-5")
        hm_cen_sigmoid = _sigmoid(outputs['hm_cen'])
        cen_offset_sigmoid = _sigmoid(outputs['cen_offset'])
        detections = decode(hm_cen_sigmoid, cen_offset_sigmoid, outputs['direction'], outputs['z_coor'],
    ...
    for detection in outputs:
        ...
        x = configs.lim_x[0] + (configs.lim_x[1] - configs.lim_x[0]) * detection[0]
        y = configs.lim_y[0] + (configs.lim_y[1] - configs.lim_y[0]) * detection[1]
        z = configs.lim_z[0] + (configs.lim_z[1] - configs.lim_z[0]) * detection[2]
        w = configs.lim_x[0] + (configs.lim_x[1] - configs.lim_x[0]) * detection[3]
        h = configs.lim_y[0] + (configs.lim_y[1] - configs.lim_y[0]) * detection[4]
        l = configs.lim_z[0] + (configs.lim_z[1] - configs.lim_z[0]) * detection[5]
        yaw = detection[6]

        ## step 4 : append the current object to the 'objects' array
        objects.append([x, y, z, w, h, l, yaw])
    #######
    ####### ID_S3_EX2 START #######   
